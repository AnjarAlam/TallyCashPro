...
export function CopyCategoryDialog({
  open,
  onOpenChange,
  businessId,
  targetBookId,
}: CopyCategoryDialogProps) {
  ...
  const { copyCategories, isCopyingCategories } = useCopyCategories();
  ...
  const filteredBooks = availableBooks.filter((book) => {
    ...
    return (
      ...
      book.companyName.toLowerCase().includes(searchLower) ||
      ...
      <SheetContent side="right" className="w-full sm:max-w-[450px] flex flex-col p-0">
        ...
        <SheetHeader className="px-4 py-4 border-b">
          ...
          <SheetDescription className="text-sm text-gray-600">
            Select a book to copy all categories from
          </SheetDescription>
        </SheetHeader>
        ...
        <SheetFooter className="px-4 py-4 border-t bg-white mt-auto gap-2">
          <Button
            ...
            disabled={!selectedSourceBookId || isCopyingCategories}
            className="w-full"
          >
            {isCopyingCategories ? (
              <>
